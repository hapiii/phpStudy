...
if($do == "delete"){
    $id = $_GET['id'];
    $result = $conn->query("delete from voteoption where cid in ($id);");
    if($result){
...
if(isset($_POST['Submit'])?$_POST['Submit']:''){
    ...
    $result = $conn->query("update voteoption set optionname='$optionname', votenum='$votenum', upid='$subject' where cid='$id';");
    if($result){
...
<div class="div_from_aoto" style="width: 800px; margin:30px 40px;">
    ...
    <form name="form1" method="post" action="">
        <table width="560" border="0" cellspacing="0" cellpadding="0">
            ...
            <?php
            $result = mysqli_query($conn,"select * from voteoption;");
            while($row = mysqli_fetch_assoc($result)){
                ...
                <tr>
                    ...
                    <td width="34"><input style="width:40px; height:22px;" value="修改" type="button" onClick="location.href='?do=change&id=<?php echo $row['cid']; ?>'"></td>
                    <td width="34"><input style="width:40px; height:22px; color: #7d0000" value="删除" type="button" onClick="location.href='?do=delete&id=<?php echo $row['cid']; ?>'"></td>
                </tr>
            <?php }?>
            <tr>
    ...
    if( $do == "change"){
        $id = $_GET['id'];
        $result = mysqli_query($conn,"select * from voteoption where cid='$id';");
            ...
            <form action="" method="post">
                ...
                <label>
                    <select style="width:300px;" name="subject">
                        <?php
                        $result2 = $conn->query("select * from votename");
